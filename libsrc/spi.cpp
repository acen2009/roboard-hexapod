...
// note: this function is only accurate on RB-100/RB-110/RB-050 that use 300MHz as DRAM clock
_RB_INLINE unsigned char get_spidivisor(int mode) {
    static unsigned long SPI_clkTable[15] = {
        21000000L, 18750000L, 15000000L, 12500000L, 10000000L, 
        10700000L, 11500000L, 13600000L, 16600000L, 25000000L, 
        30000000L, 37000000L, 50000000L, 75000000L, 150000000L 
    };
    unsigned long spiclk = SPI_clkTable[mode], dramclk = io_GetDRAMCLK() * 1000000L;
    unsigned long i, clkerr, lasterr = 0xffffffffL;

    for (i=1L; i<16L; i++)
    {
        clkerr = dramclk / (2L * i);
        clkerr = (spiclk > clkerr)? spiclk - clkerr : clkerr - spiclk;
        if (clkerr > lasterr) break; else lasterr = clkerr;
    }
    
    return (unsigned char)(i-1L);
}
...
RBAPI(bool) spi_EnableSS(void) {
    if ((roboio_GetRBVer() != RB_100RD) && (roboio_GetRBVer() != RB_100) &&
        (roboio_GetRBVer() != RB_100b1) && (roboio_GetRBVer() != RB_100b2) && (roboio_GetRBVer() != RB_100b3))
    {
