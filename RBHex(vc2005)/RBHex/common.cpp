...
#include "common.h"
    ...
    // RoBoIO-dedicated timer functions
    RBAPI(void) roboio_Delay(unsigned long delaytime) {    // delay in ms
        delay_ms(delaytime);
    }

    RBAPI(void) roboio_DelayUs(unsigned long delaytime) {  // delay in us
        for (; delaytime > 100000L; delaytime = delaytime - 100000L) delay_us(100000L);
        if (delaytime > 0L) delay_us(delaytime);
    }
    
    RBAPI(unsigned long) roboio_GetTick(void) {  // get the current time-ticks in 1ms resolution
...
RBAPI(unsigned long) timer_nowtime(void) { //in ms
    ...
    */
    ...
    unsigned long CLOCKS_PER_MILLISEC = io_GetCPUCLK() * 1000UL;
    int i;
    ...
    __emit__(0x0f, 0x31);
    __emit__(0x66); _asm mov WORD PTR nowclocks_l, ax;
    __emit__(0x66); _asm mov WORD PTR nowclocks_h, dx;

    __emit__(0x66); _asm pop dx;
    __emit__(0x66); _asm pop ax;

    // compute nowtime = (unsigned long)(nowclocks/CLOCKS_PER_MILLISEC), 
    // assuming CLOCKS_PER_MILLISEC is a 24-bit integer and using a stupid but simple code:p
    nowtime     = nowclocks_h / CLOCKS_PER_MILLISEC;
    nowclocks_h = nowclocks_h % CLOCKS_PER_MILLISEC;
    for (i=3; i>=0; i--)
    {
        nowclocks_h = (nowclocks_h << 8) + (unsigned long)((unsigned char*)&nowclocks_l)[i];
        nowtime     = nowtime << 8;
        if (nowclocks_h >= CLOCKS_PER_MILLISEC)
        {
            nowtime     = nowtime + (nowclocks_h / CLOCKS_PER_MILLISEC);
            nowclocks_h = nowclocks_h % CLOCKS_PER_MILLISEC;
        }
    }
    ...
    return (unsigned long)((uclock() - inittime)*(uclock_t)1000UL/UCLOCKS_PER_SEC);
#elif defined(RB_WATCOM)
    static bool usetimer = false;
    static unsigned long long inittime;
    ...
    if (usetimer == false)
    {
        inittime = (unsigned long long)clock();
        usetimer = true;
    }
...
RBAPI(void) delay_ms(unsigned long delaytime) { //delay in ms
	...
	*/
    ...
    for (; delaytime > 100L; delaytime = delaytime - 100L) delay_us(100000L);
    if (delaytime > 0L) delay_us(delaytime * 1000UL);
#endif
}
